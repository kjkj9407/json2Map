package com.fenbi.pumpkin.science.admin.server;
/**
 *
 */

import com.fenbi.common.util.JsonUtils;
import com.fenbi.pumpkin.science.admin.server.web.data.vo.question.SingleQuestionVO;
import com.fenbi.pumpkin.science.refactor.data.question.SingleOption;
...
public class JsonParseUtil {
        ...
        for (Map.Entry<String, JsonElement> object : jsonObject.entrySet()) {
            ...
            if (value.isJsonNull()) {
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    parseJson2Map(map, jsonArray.get(i).getAsJsonObject(), fullkey+"_"+i);
                }
                continue;
    ...
    public static void parseJson2Map(Map map, String json, String parentKey) {
        ...
        if (jsonElement.isJsonObject()) {
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                parseJson2Map(map, jsonArray.get(i).getAsJsonObject(), parentKey+"_"+i);
            }
...
//        System.out.println(jsonFormatter(test));
        ...
        SingleQuestionVO singleQuestionVO = new SingleQuestionVO();
        ArrayList<SingleOption> singleOptions = new ArrayList<>();
        SingleOption op1 = new SingleOption();
        op1.setContent("op1");
        SingleOption op2 = new SingleOption();
        op2.setContent("op2");
        singleOptions.add(op1);
        singleOptions.add(op2);

        singleQuestionVO.setOptions(singleOptions);
        System.out.println(jsonFormatter(JsonUtils.writeValue(singleQuestionVO)));
